/**
 * DentalFlow — Ruta: Appointments (multi-tenant)
 * Todas las queries estan scopeadas a req.user.id.
 */
...
/** Verifica solapamiento solo dentro de las citas del usuario */
async function verificarConflicto(fechaHoraInicio, duracionMinutos, excludeId, userId) {
...
router.get('/today', async (req, res) => {
  try {
    ...
    const data = await db.prepare(`
      ...
      FROM appointments a
      JOIN patients p ON a.patient_id = p.id
      WHERE a.fecha_hora_inicio LIKE ? AND a.user_id = ?
      ORDER BY a.fecha_hora_inicio ASC
    `).all(hoy + '%', req.user.id);
...
router.get('/upcoming', async (req, res) => {
  try {
    ...
    `).all(ahora, en30dias, req.user.id);
...
router.get('/slots/:fecha', async (req, res) => {
  try {
    ...
    const citas = await db.prepare(`
      ...
      FROM appointments a JOIN patients p ON p.id = a.patient_id
      WHERE a.fecha_hora_inicio LIKE ? AND a.estado NOT IN ('cancelada','no_asistio') AND a.user_id = ?
      ORDER BY a.fecha_hora_inicio ASC
    `).all(fecha + '%', req.user.id);
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const messages = await db.prepare(
      'SELECT * FROM message_log WHERE appointment_id = ? ORDER BY created_at ASC'
    ).all(req.params.id);
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!duracion_minutos || duracion_minutos < 15) return res.status(400).json({ error: 'La duracion minima es 15 minutos' });
    if (isNaN(new Date(fecha_hora_inicio).getTime())) return res.status(400).json({ error: 'Formato de fecha invalido' });

    if ((!patient_id || patient_id === 'new') && nombre && telefono) {
      const tel = telefono.replace(/\D/g, '');
      let patient = await db.prepare('SELECT * FROM patients WHERE telefono = ? AND user_id = ?').get(tel, uid);
      if (!patient) {
        const r = await db.prepare('INSERT INTO patients (nombre, telefono, user_id) VALUES (?, ?, ?)').run(nombre.trim(), tel, uid);
      ...
      if (!await db.prepare('SELECT id FROM patients WHERE id = ? AND user_id = ?').get(patient_id, uid)) {
    ...
    if (conflicto) {
      ...
      return res.status(409).json({
        ...
        message: 'Ya hay una cita con ' + conflicto.paciente_nombre + ' de ' + formatHora(conflicto.fecha_hora_inicio) + ' a ' + formatHora(fin.toISOString()) + ' que se solaparia.',
    ...
    res.status(201).json({ data: newAppt, message: 'Cita creada. Recordatorios automaticos programados.' });
  } catch (err) {
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const appt = await db.prepare('SELECT * FROM appointments WHERE id = ? AND user_id = ?').get(id, uid);
    if (!appt) return res.status(404).json({ error: 'Cita no encontrada' });

    if (fecha_hora_inicio && fecha_hora_inicio !== appt.fecha_hora_inicio) {
      const conflicto = await verificarConflicto(fecha_hora_inicio, duracion_minutos || appt.duracion_minutos, parseInt(id), uid);
...
router.delete('/:id', async (req, res) => {
  try {
    const appt = await db.prepare('SELECT id FROM appointments WHERE id = ? AND user_id = ?').get(req.params.id, req.user.id);
    if (!appt) return res.status(404).json({ error: 'Cita no encontrada' });
